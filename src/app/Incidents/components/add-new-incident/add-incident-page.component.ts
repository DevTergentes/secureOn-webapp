...
export class AddIncidentPageComponent implements OnInit {
  ...
  private initializeDateLimits(): void {
    ...
    console.log('Fecha actual en Lima:', limaTime);
    
    // Solo permitir fechas del año actual (2025)
    const startOfYear = new Date(2025, 0, 1); // 1 de enero de 2025
    const endOfYear = new Date(2025, 11, 31); // 31 de diciembre de 2025
    ...
    console.log('Límites de fecha establecidos - Min:', this.minDate, 'Max:', this.maxDate);
    console.log('Fecha por defecto establecida:', this.incident.date);
  }
  ...
  validateDate(): void {
    ...
    console.log('Validando fecha:', selectedDate, 'vs max permitida:', maxAllowedDate);
